...
namespace physics {
  ...
  float _toiTime, _toiMaxTime;
  int32_t _toiCalls, _toiIters, _toiMaxIters;
  int32_t _toiRootIters, _toiMaxRootIters;
  ...
  struct SeparationFunction {
    ...
    float Initialize(const SimplexCache* cache,
      ...
      if (count == 1) {
        ...
        Vec2 localPoint = proxyB->GetVertex(cache->indexB[1]);
        
        m_axis = Cross(localPoint - localPointB1, 1.0f);
        m_axis.Normalize();
  ...
  void TimeOfImpact(TOIOutput* output, const TOIInput* input)
  {
    ...
    float totalRadius = proxyA->radius_ + proxyB->radius_;
    float target = Max(LinearSlop, totalRadius - 3.0f * LinearSlop);
    float tolerance = 0.25f * LinearSlop;
    IK_ASSERT(target > tolerance);
    
    float t1 = 0.0f;
    const int32_t k_maxIterations = 20;  // TODO_ERIN Settings
    int32_t iter = 0;
    ...
    for(;;) {
      ...
      for (;;) {
        ...
        _toiMaxRootIters = Max(_toiMaxRootIters, rootIterCount);
    ...
    _toiMaxIters = Max(_toiMaxIters, iter);
    
    float time = timer.GetMilliseconds();
    _toiMaxTime = Max(_toiMaxTime, time);
    _toiTime += time;
  }
