...
namespace ikan {
  ...
  MacWindow::MacWindow(const Window::Specification& specification) {
    IK_CORE_INFO("Creating MAC OS Window instacne ... ");
  ...
  MacWindow::~MacWindow() {
    IK_CORE_WARN("Destroying MAC OS Window instacne !!! ");
    Shutdown();
  }
...
[](
  ...
  void MacWindow::Shutdown() {
    IK_CORE_WARN("Shutting down the MAC OS Window");
    glfwTerminate();
  ...
  void MacWindow::Maximize() {
    IK_CORE_INFO("Maximising the window");
    glfwMaximizeWindow(window_);
  }
  
  void MacWindow::CenterWindow() {
    IK_CORE_INFO("Placing the window at the center");
  ...
  void MacWindow::SetResizable(bool resizable) const {
    if (resizable)
      IK_CORE_INFO("Window can be resized");
    else
      IK_CORE_INFO("Window can NOT be resized");
  ...
  void MacWindow::SetTitle(const std::string& title) {
    IK_CORE_INFO("New MAC Window Title is : {0} (WARNING: Window specificaiton"
