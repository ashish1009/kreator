...
namespace physics {
  ...
  /// A shape is used for collision detection. You can create a shape however you like. Shapes used
  /// for simulation in World are created automatically when a Fixture is created. Shapes may encapsulate
  /// a one or more child shapes.
  class Shape {
  public:
    ...
    virtual bool TestPoint(const Transform2D& xf, const Vec2& p) const = 0;
    ...
    virtual bool RayCast(RayCastOutput* output, const RayCastInput& input,
                         const Transform2D& transform, int32_t child_index) const = 0;
    ...
    virtual void ComputeAABB(AABB* aabb, const Transform2D& xf, int32_t child_index) const = 0;
    ...
    virtual void ComputeMass(MassData* mass_data, float density) const = 0;
